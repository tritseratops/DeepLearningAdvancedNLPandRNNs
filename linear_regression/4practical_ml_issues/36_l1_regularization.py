...
for i in range(4, D+1):
    col_name = column_name_base + str(i)
    df[col_name] = np.random.randint(min, max, df.shape[0]) # noise
...
# X = np.vstack([np.ones(X_all.shape[0]), X_all]).T
X = X_all
